"""Estrategia de scraping para Gaceta Parlamentaria (Diputados) — paramétrico por legislatura."""
...
def _get_base_url_for_legislature(legislature: str) -> str:
    """Obtiene la URL base de Gaceta para una legislatura desde config."""
    config = _load_config()
    leg_config = config.get("legislatures", {}).get(legislature, {})
    return leg_config.get("urls", {}).get("diputados", {}).get("gaceta_base", "")
...
def scrape_gaceta(
    ...
    """Captura tablas agregadas y POSTs nominales de la Gaceta Parlamentaria.
    ...
    """
    base_url = _get_base_url_for_legislature(legislature)
    if not base_url:
        logger.error("No hay URL de Gaceta configurada para legislatura %s", legislature)
        return {
            "source": f"gaceta_{legislature.lower()}",
            "error_fatal": f"Sin config de Gaceta para {legislature}",
        }
    ...
    for numero in tabla_range:
        ...
        try:
            tabla_url, fetch_result = _fetch_tabla(engine, base_url, numero)
...
def scrape_gaceta_lxvi(
    ...
    """Backward compatible wrapper — delegates to :func:`scrape_gaceta` with legislature=LXVI."""
    return scrape_gaceta(
        engine,
        persistence,
        legislature="LXVI",
        since=since,
