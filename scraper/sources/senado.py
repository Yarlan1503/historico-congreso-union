"""Estrategia de scraping para Senado — paramétrico por legislatura.
...
def _load_config() -> dict:
    """Carga la config TOML del scraper (cacheada por call-site)."""
    with open(_CONFIG_PATH, "rb") as fh:
        return tomllib.load(fh)


def _get_senado_urls(legislature: str) -> dict[str, str]:
    """Obtiene las URLs del Senado para una legislatura desde config."""
    config = _load_config()
    leg_config = config.get("legislatures", {}).get(legislature, {})
    return leg_config.get("urls", {}).get("senado", {})


# ---------------------------------------------------------------------------
# Helpers compartidos (mismos que senado_lxvi.py original)
# ---------------------------------------------------------------------------
...
def _derive_source_tags(legislature: str) -> tuple[str, str]:
    """Deriva los source_tags (ajax, html) para una legislatura.

    LXVI usa los tags legacy ``sen_lxvi_ajax`` / ``sen_lxvi_html``.
    Otras legislaturas usan tags dinámicos ``sen_{leg}_ajax`` / ``sen_{leg}_html``.
    """
    if legislature == "LXVI":
        return "sen_lxvi_ajax", "sen_lxvi_html"
    return f"sen_{legislature.lower()}_ajax", f"sen_{legislature.lower()}_html"
...
def scrape_senado(
    ...
    """Scrapea votaciones del Senado para una legislatura específica.
    ...
    """
    urls = _get_senado_urls(legislature)
    if not urls:
        return {
            "source": f"senado_{legislature.lower()}",
            "error_fatal": f"Sin config para legislatura {legislature}",
        }

    ajax_url_tpl = urls.get("ajax", "")
    html_url_tpl = urls.get("html", "")

    if not ajax_url_tpl and not html_url_tpl:
        return {
            "source": f"senado_{legislature.lower()}",
            "error_fatal": f"Sin URLs ajax/html para legislatura {legislature}",
        }

    ajax_tag, html_tag = _derive_source_tags(legislature)
    ...
    for vid in id_range:
        ...
        logger.info("[%s] Procesando votación ID %s", legislature, vid)

        # ------------------------------------------------------------------
        # AJAX primario
        # ------------------------------------------------------------------
        ajax_url = ajax_url_tpl.format(votacion_id=vid) if ajax_url_tpl else ""
        ...
        if ajax_url:
            try:
                fetch_ajax = engine.fetch(
                    ajax_url, method="GET", source_tag=ajax_tag
                )
            except Exception as exc:
                logger.exception("[%s] Excepción en fetch AJAX ID %s", legislature, vid)
                stats["errores"].append(f"ID {vid} AJAX fetch exc: {exc}")

            if fetch_ajax is not None:
                try:
                    process_ajax = pipeline.process(
                        fetch_ajax, source_tag=ajax_tag
                    )
                except Exception as exc:
                    logger.exception("[%s] Excepción en pipeline AJAX ID %s", legislature, vid)
        ...
        if fetch_ajax is not None and process_ajax is not None:
            if _is_waf(fetch_ajax, process_ajax):
                try:
                    pr = persistence.persist(process_ajax, ajax_tag)
                    _update_persist_stats(pr, stats, is_success=False)
                except Exception as exc:
                    logger.exception("[%s] Persist WAF AJAX ID %s error", legislature, vid)
                    stats["errores"].append(f"ID {vid} AJAX WAF persist: {exc}")
                ...
                if _should_persist(process_ajax, since):
                    try:
                        ...
                        logger.exception("[%s] Persist AJAX SUCCESS ID %s error", legislature, vid)
                        stats["errores"].append(f"ID {vid} AJAX persist: {exc}")
                else:
                    logger.info("[%s] ID %s AJAX omitido por filtro since", legislature, vid)
                ...
                try:
                    ...
                    logger.exception("[%s] Persist AJAX failed ID %s error", legislature, vid)
                    stats["errores"].append(f"ID {vid} AJAX failed persist: {exc}")
        ...
        if ajax_finalized:
            if consecutive_not_found >= break_on_consecutive_not_found:
                logger.warning(
                    "[%s] Break tras %s IDs consecutivos no encontrados (umbral %s)",
                    legislature, consecutive_not_found, break_on_consecutive_not_found,
                )
        ...
        # HTML fallback
        # ------------------------------------------------------------------
        html_url = html_url_tpl.format(votacion_id=vid) if html_url_tpl else ""
        ...
        if html_url:
            try:
                fetch_html = engine.fetch(
                    html_url, method="GET", source_tag=html_tag
                )
            except Exception as exc:
                logger.exception("[%s] Excepción en fetch HTML ID %s", legislature, vid)
                stats["errores"].append(f"ID {vid} HTML fetch exc: {exc}")

            if fetch_html is not None:
                try:
                    process_html = pipeline.process(
                        fetch_html, source_tag=html_tag
                    )
                except Exception as exc:
                    logger.exception("[%s] Excepción en pipeline HTML ID %s", legislature, vid)
                    stats["errores"].append(f"ID {vid} HTML pipeline exc: {exc}")
        ...
        if fetch_html is not None and process_html is not None:
            if _is_waf(fetch_html, process_html):
                try:
                    pr = persistence.persist(process_html, html_tag)
                    _update_persist_stats(pr, stats, is_success=False)
                except Exception as exc:
                    logger.exception("[%s] Persist WAF HTML ID %s error", legislature, vid)
                    stats["errores"].append(f"ID {vid} HTML WAF persist: {exc}")
                ...
                if _should_persist(process_html, since):
                    try:
                        ...
                        logger.exception("[%s] Persist HTML SUCCESS ID %s error", legislature, vid)
                        stats["errores"].append(f"ID {vid} HTML persist: {exc}")
                else:
                    logger.info("[%s] ID %s HTML omitido por filtro since", legislature, vid)
                ...
                try:
                    ...
                    logger.exception("[%s] Persist HTML failed ID %s error", legislature, vid)
                    stats["errores"].append(f"ID {vid} HTML failed persist: {exc}")
