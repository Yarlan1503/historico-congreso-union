...
RAW_V0_1_SCHEMA_PATH = Path(
    "/home/cachorro/Documentos/Proyectos/popolo-congreso-union/schemas/sql/raw_v0_1.sql"
)
...
# chamber constante para raw.db — siempre 'diputados' en este export.
_CHAMBER = "diputados"
...
def create_raw_db(
    ...
    Args:
        ...
        chamber_source: Cámara en formato source ('diputados').
    ...
    try:
        ...
        for (legislator_name,) in person_names:
            ...
            conn.execute(
                ...
                (
                    ...
                    None,
                    None,
                    "Person stub from legislator_name; no disambiguation performed.",
                ),
            )
        ...
        for pkey, person_id in person_key_to_id.items():
            mkey = build_membership_key(pkey, _CHAMBER, legislature)
            conn.execute(
        ...
        for ve_id, vote_date, title, subject, source_url in ve_source_rows:
            ...
            # --- raw_motion ---
            mkey = build_motion_key(_CHAMBER, legislature, src_url)
            ...
            conn.execute(
                ...
                "source_motion_id) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (mkey, _CHAMBER, legislature, motion_title, subject, None),
            )
            motion_id = _get_id(conn, "raw_motion", "motion_key", mkey)

            # --- raw_vote_event ---
            ve_key = build_vote_event_key(_CHAMBER, legislature, src_url)
